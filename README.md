...
**Note regarding FWD:** To compute the FWD metric, you need access to pre-trained Wrenformer weights. The code expects them to be located at `evaluation_checkpoint/checkpoint.pth`, and they will be downloaded automatically if not present. You can of course download these yourself: create the directory `evaluation_checkpoint` and move the weights there. The code will run a checksum to verify that they are the same weights.

**Note regarding other models:** The evaluation code also supports parsing data generated by other models that we compared to in the paper (CDVAE, DiffCSP++, SymmCD). The data will be converted to protostructures and then evaluated.
